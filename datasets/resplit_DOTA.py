...
3. output: 2 .csv files: train.csv and test.csv
    <image_path class-id >
...
def merge_data(root):
    ...
    all.to_csv(root+'all.csv', sep=' ', header=None, index=False)



def split_dataset(root):
    merge_data(root)
    data = pd.read_csv(root + 'all.csv', header=None, sep=' ')
    x = data.iloc[:]
    x_train, x_test = train_test_split(x, test_size=0.3, random_state=0, )
    x_train.to_csv('/home/dingjin/HiwiJob/example/train.csv', sep=' ', header=None, index=False)
    x_test.to_csv('/home/dingjin/HiwiJob/example/test.csv', sep=' ', header=None, index=False)
