...
import numpy as np
import math
from utils.dota_utils import GetFileFromThisRootDir
...
class splitDOTA():
    ...
    def __init__(self, basepath,
        ...
        self.deletelabelpath = os.path.join(outpath, 'labelTxt')
        self.deleteimagepath = os.path.join(outpath, 'images')
    ...
    def saveimagepatches(self, img, subimgname, left, up):
        ...
        if (self.padding):
            ...
            plt.imsave(outdir, outimg) ###
        else:
            plt.imsave(outdir, subimg) ###
    ...
    def SplitSingle(self, name, rate, extent):
        ...
        img = plt.imread(os.path.join(self.imagepath, name + extent))
        if np.shape(img) == ():
            return
    ...
    def deleteinfo(self):
        ...
        :return:
        """
        labellist = GetFileFromThisRootDir(self.deletelabelpath)
        files = list(filter(lambda file: os.stat(file).st_size > 0, labellist))
        for label in files:
            lines_seen = set()
    ...
    def clean(self):
        """
        delete empty txt and co-responding image
        :return:
        """
        labellist = GetFileFromThisRootDir(self.deletelabelpath)
        files = list(filter(lambda file: os.stat(file).st_size == 0, labellist))
        for label in files:
            os.remove(label)
            os.remove(os.path.join(self.deleteimagepath, os.path.split(label)[1].split(".")[0] + ".png"))
